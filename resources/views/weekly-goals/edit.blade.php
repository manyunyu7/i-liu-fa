<x-app-layout>
    <x-slot name="title">Edit Weekly Goal</x-slot>
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="mb-6">
            ...
            <h1 class="text-2xl font-extrabold text-duo-gray-500">Edit Weekly Goal</h1>
            <p class="text-duo-gray-300 mt-1">Update your goal details</p>
        </div>

        <!-- Form -->
        <x-card>
            <form action="{{ route('weekly-goals.update', $weeklyGoal) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $weeklyGoal->title) }}"
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="w-full px-4 py-3 rounded-duo border-2 border-duo-gray-100 focus:border-duo-green focus:ring-0 font-medium">{{ old('description', $weeklyGoal->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-duo-red">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        @foreach($categories as $key => $category)
                            <label class="cursor-pointer">
                                <input type="radio" name="category" value="{{ $key }}"
                                       class="sr-only peer" {{ old('category', $weeklyGoal->category) === $key ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-4" x-data="{ count: {{ old('target_count', $weeklyGoal->target_count) }} }">
                ...
                <div class="bg-duo-gray-50 rounded-duo p-4">
                    <p class="text-sm text-duo-gray-400 mb-2">Current Progress</p>
                    <div class="flex items-center space-x-4">
                        <x-progress-bar :progress="$weeklyGoal->progress_percentage" color="green" class="flex-1" />
                        <span class="font-bold text-duo-gray-500">{{ $weeklyGoal->current_count }}/{{ $weeklyGoal->target_count }}</span>
                    </div>
                </div>
